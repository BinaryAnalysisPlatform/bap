...
module Check = struct
  ...
  let run {path; ida} =
    let require_kind k =
      ...
      require_path ~error:(File_not_found file) file in
    Result.all_ignore [
        require_ida path;

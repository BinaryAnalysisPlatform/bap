...
let fold ?(word_size=`r8) t ~(init:'a) ~f : 'a =
  let read = (getter t word_size).fast in
  let pos_ref = ref t.off in
  let word_len = Size.to_bytes word_size in
  let finish = t.off + t.size in
  let rec loop init =
    if pos_ref.contents + word_len <= finish then
      let addr = Addr.(t.addr ++ (!pos_ref - t.off))  in
      loop (f addr (read ~pos_ref) init)
    else init in
  loop init
...
let pp fmt t =
  ...
  let chars = fold t ~init:[] ~f:(fun addr char chars ->
...
let iter ?word_size t ~f =
  fold ?word_size t ~init:() ~f:(fun a w () -> f a w)

let exists ?word_size t ~f =
  with_return (fun s -> iter ?word_size t ~f:(fun a w ->
...
let count ?word_size t ~f =
  fold ?word_size t ~init:0 ~f:(fun a w n -> if f a w then n + 1 else n)

let find_map ?word_size t ~f =
  with_return (fun s -> iter ?word_size t ~f:(fun a w -> match f a w with
...
module Make_iterators( M : Monad.S) = struct
  ...
  let fold ?word_size t ~(init:'a) ~f : 'a m =
    fold ?word_size t ~init:(return init) ~f:(fun a w s -> s >>= f a w)
  ...
  let exists ?word_size t ~f =
    with_return (fun s ->
        iter ?word_size t ~f:(fun a x -> f a x >>= function
  ...
  let count ?word_size t ~f =
    fold ?word_size t ~init:0 ~f:(fun a w s ->
        f a w >>| function
        | true  -> s + 1
        | false -> s)
  ...
  let find_map ?word_size t ~f =
    with_return (fun s ->
        iter t ~f:(fun a w -> f a w >>= function
          | None -> return ()

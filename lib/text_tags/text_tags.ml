...
module Html = struct
  open Sexp
  let mark_open_stag tag : string = match tag with
    | String_tag tag ->
      begin match Sexp.of_string tag with
        | List (Atom tag :: attrs) ->
  ...
  let mark_close_stag tag : string = match tag with
    | String_tag tag ->
      begin match Sexp.of_string tag with
...
module Blocks = struct
  ...
  let filter_attrs tag : string option = match tag with
    | String_tag tag ->
      begin match Sexp.of_string tag with
        | List (Atom _ :: attrs) ->
...
module Attr = struct
  ...
  let ascii_color = function
    | String_tag tag ->
      begin try Option.some @@ match name_of_attr tag with
  ...
  let need_to_print = function
    | String_tag tag -> Hash_set.mem attrs (name_of_attr tag)
    | _ -> expect_stag ()
  ...
  let print_open_stag ppf tag : unit =
    if need_to_print tag then
      match ascii_color tag, tag with
      | Some color, _ when colorify.contents ->
